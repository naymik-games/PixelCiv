...
class buildUnit extends Phaser.Scene {
  ...
  buildImprovement() {
    ...
    this.Main.addUnit(this.select.owner, this.select.city, this.Main.selectedTile, this.buildButton.type, false)
    this.showCurrentUnits(0, this.select.city, this.select.id)
    //  console.log(theGame.countries[0].cities[this.select.city].improvements.length)
  }
  ...
  showCurrentUnits(owner, city, tileID) {
    let impr = theGame.countries[owner].cities[city].units
    for (let i = 0; i < impr.length; i++) {
      if (!impr[i].placed) {
        ...
        var timeLeft = (theGame.day + unitInfo[unit.id].days) - ((unit.turnAdded == undefined) ? theGame.day : unit.turnAdded)
